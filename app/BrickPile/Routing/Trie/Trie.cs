...
namespace BrickPile.Routing.Trie
{
    public class Trie : IDictionary<string, TrieNode>
    {
        ...
        public IEnumerable<KeyValuePair<string, TrieNode>> ChildrenOf(TrieNode parent)
        {
            return ChildrenOf(parent.PageId);
        }

        public IEnumerable<KeyValuePair<string, TrieNode>> ChildrenOf(string key)
        {
            return nodes.Where(x => x.Key != key && x.Key.StartsWith(key));
        }
